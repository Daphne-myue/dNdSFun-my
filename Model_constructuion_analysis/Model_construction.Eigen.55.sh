################################################################################################################
# step 1 : Construct models of Eigen scores
...
mkdir -p ${wkDir}/${gencode_anno}/Eigen_score

# Set chromosome variable
chr="chr${id}"
outfile="${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.excDupSite.txt"
...
# Filter sites based on the chromosome, process them with `tabix` for Eigen scoring, and filter using `bedtools`
cat ${bed} | awk -v var=$chr '{if($1==var)print $1":"$2+1"-"$3}' | sort -k 1,1 | uniq | xargs tabix /path/eigen/chr/hg19_eigen_${chr}.txt.gz | awk 'BEGIN{OFS="\t"}{print $1,$2-1,$2,$4,$5,$6}' | bedtools intersect -a stdin -b ${bed} -wo | awk 'BEGIN{OFS="\t"}{split($10,a,"::");print a[4],a[3],$10,$1,$3,$4,$5,$6,$6,$6,$8+1,$9,$12}' | sort | uniq >>${outfile} 

rm ${wkDir}/${gencode_anno}/Eigen_score/${chr}_o
cat ${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.excDupSite.txt | cut -f8 | grep -v "RawScore" >>${wkDir}/${gencode_anno}/Eigen_score/${chr}_o
cat ${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.excDupSite.txt | cut -f 2,4,5,6,7,8,9,10 | grep --color=auto -v ^"gene.name" > ${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.impScore.txt

################################################################################################################
# step 2 : Get the dichotomy threshold of all mutations using Eigen scores
...
cat ${runDir}/QuantSet_${neutral}_${quant}/${gencode_anno}/Eigen_score/chr*_o > ${runDir}/QuantSet_${neutral}_${quant}/${gencode_anno}/Eigen_score/total_o
threshold_positive=`Rscript ./quantile.R ./QuantSet_50_0.50/gc19_pc.${reg}/Eigen_score/total_o ${quant}`
...
################################################################################################################
# step 3 : Construct the dNdS-Fun model for each chromosome of Eigen scores
################################################################################################################
#!/bin/bash
#SBATCH -N 1
...
elementfile="./QuantSet_${neutral}_0.${neutral}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.excDupSite.txt"
genomefile="/path/GRCh37.primary_assembly.genome.fasta"
outfile="${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_${chr}.rda"

Rscript /path/buildRefElement_Eigen_score_rda.R ${elementfile} ${genomefile} ${outfile} ${chr} RawScore ${threshold_positive} ${threshold_neutral}
echo "run done"
...
prefix="${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_"
outfile="${wkDir}/${gencode_anno}/Eigen_score/${gencode_anno}_Eigen_score_ALL_${quant}.rda"
