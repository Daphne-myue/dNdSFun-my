################################################################################################################
# step 1 : split the samples of GEL data into difference cancer subtypes
################################################################################################################
#!/bin/bash
#SBATCH -N 1
...
grep -w ${tissue} singleproject_single/GEL_SNVs_allmutations.bed > singleproject_single/GEL_SNVs_allmutations.${tissue}.bed
...
for chr in {1..22}
do
   Rscript S00_Add_impScore.GEL.R /path/CADD_Score/${gencode_anno}/${gencode_anno}_CADD_score.impScore.${chr}.bed splitchr/GEL_SNVs_allmutations.${chr}.txt splitchr/GEL_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.${chr}.txt
done

cat splitchr/GEL_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.${chr}.txt > splitchr/GEL_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.all.txt
