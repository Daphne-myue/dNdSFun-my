################################################################################################################
# step 1 : split the samples of PCAWG data into difference cancer subtypes
################################################################################################################
#!/bin/bash
#SBATCH -N 1
...
for chr in {1..22}
do
   Rscript S00_Add_impScore.PCAWG.R /path/CADD_Score/${gencode_anno}/${gencode_anno}_CADD_score.impScore.${chr}.bed splitchr/PCAWG_SNVs_allmutations.${chr}.txt splitchr/PCAWG_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.${chr}.txt
done

cat splitchr/PCAWG_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.${chr}.txt > splitchr/PCAWG_SNVs_allmutations.txt.${gencode_anno}_CADD_score.impScore.all.txt
